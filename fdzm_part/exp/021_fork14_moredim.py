...
class LeapModel(nn.Module):
    def __init__(self, inputs_dim=25, num_lstm=8, hidden_state=300):
        super().__init__()
        ...
        for i in range(num_lstm):  # 假设有5个LSTM层
            ...
            if i == 0:
                residual_layers.append(nn.Sequential(nn.LayerNorm([60, hidden_state*2]),nn.GELU()))
            else:
                residual_layers.append(nn.LayerNorm([60, hidden_state*2]),)
    ...
    def forward(self, inputs):
        ...
        for i, lstm in enumerate(self.lstm_stack.values()):
            ...
            if i > 0:
                outputs = self.res_act(0.7*outputs+0.3*last_outputs)  # 残差连接
                last_outputs = outputs
            else:  # i % 2 ==0
                last_outputs = outputs  # 存一下上两层的预测
...
if __name__ == "__main__":
    ...
    model = nn.DataParallel(LeapModel().to(CFG.device))
    print(model)
    tmp_model = LeapModel()
    ...
    criterion = LEAPLoss()#MSELoss()
    score_func = negative_r2

    oof_npy, best_score, best_zero_list = train_loop(train_loader, val_loader, model, optimizer, scheduler, criterion,
                                                     score_func)
