...
config = {
    ...
    "cosine_n_cycles": 0.5,
    "learning_rate": 1e-4,
    "resume": "../outputs/forcewithme_2LSTM1mamba-3_state64_cv0.7893_LB7868/forcewithme_2LSTM1mamba-3_state64_cv0.7893_LB7868.pt",
    "gid2skip": 0,
}
...
class LeapModel(nn.Module):
    def __init__(self, inputs_dim=25, num_lstm=3, hidden_state=256, d_state=64, d_conv=4, expand=2):
        super().__init__()
        ...
        for i in range(num_lstm):  # 假设有5个LSTM层
            lstm_key = f'lstm{i + 1}'
            self.lstm_layers[lstm_key] = nn.Sequential(LSTM_BLOCK(hidden_state*2, num_lstm=2,
...
if __name__ == "__main__":
    ...
    if CFG.gid2skip is not None:
        # non_pred_list += zero_list_tmp
        for _ in zero_list_tmp:
            idx = target_cols.index(_)
            oof_pred[:,idx] = -mean_dict[_]/std_dict[_]
        oof_pred_skip = oof_pred[:,GIDX_DICT[CFG.gid2skip]]
        np.save(f"../outputs/{exp_id}/oof_g{CFG.gid2skip}.npy",oof_pred_skip)

    for gid in range(7):
        if CFG.gid2skip is not None:
            if gid == CFG.gid2skip: 
                print(f'Skip Group {gid}')
                continue
        ...
        #     model = nn.DataParallel(model.to(CFG.device), device_ids=list(range(3)))
        #     model.load_state_dict(torch.load(CFG.resume), strict=True)
        model = nn.DataParallel(model.to(CFG.device), device_ids=[0,1,2,3])
        model.load_state_dict(torch.load(CFG.resume), strict=True)        
