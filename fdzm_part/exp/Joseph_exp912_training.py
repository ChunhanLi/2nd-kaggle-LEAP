...
class LEAPLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        for i in range(6):
            ...
            loss += self.l1_loss(pred_diff, label_diff)*self.w / 6
...
def train_loop(train_loader, val_loader, model, optimizer,
    ...
    for epoch in range(CFG.epoch):
        ...
        #if epoch + 1 == 9:
        #    w = 0.1
        #    criterion = LEAPLoss(w=w)
        #    print(f"change loss weigh to {w}.")

        eval_steps = get_evaluation_steps(train_steps_per_epoch, CFG.evaluate_n_times_per_epoch)
        if epoch + 1 in (7, 8):  #(7, 8, 11), (6, 7, 8):
            eval_steps = get_evaluation_steps(train_steps_per_epoch, 4)
        if epoch + 1 == 9:  #12, 9:
            eval_steps = get_evaluation_steps(train_steps_per_epoch, 6)
        ...
        for step, (inputs, labels) in tqdm(enumerate(train_loader), desc="training", total=len(train_loader), leave=True):
            ...
            if CFG.eval_val and ((step + 1) in eval_steps):
                ...
                if CFG.save_best_model:
                    if oof_score < best_score_epoch - 0.00002:
...
def pred_func(inputs_array_path, model_path, model_new, new_zero_pred_list):
    ...
    sub_sample.to_parquet(f"../infer_outputs/Jo_ex912_cv7885.parquet")
    sub_sample.to_parquet(f"../../submission/subs/Jo_ex912_cv7885.parquet")
