...
def train_loop(train_loader, val_loader, model, optimizer,
    ...
    for epoch in range(CFG.epoch):
        ...
        for step, (inputs, labels) in tqdm(enumerate(train_loader), desc='train_one_epoch', total=len(train_loader)):
            ...
            cur_beta = max_beta - (scheduler.get_lr()[0] / base_lr) * (max_beta-min_beta)
...
if __name__ == "__main__":
    ...
    # optimizer = AdamW(model.module.parameters(), lr=CFG.learning_rate)
    # scheduler = get_scheduler(optimizer, CFG, num_train_steps=num_train_steps)
    optimizer = AdamW([
        {'params': model.module.lstml.parameters(), 'lr': 8e-4},
        {'params': model.module.lstm2.parameters(), 'lr': 10e-4},
        {'params': model.module.attention.parameters(), 'lr': 10e-4},
        {'params': model.module.fc.parameters(), 'lr': 12e-4},
    ])
