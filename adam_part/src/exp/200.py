...
class LeapModel(nn.Module):
    def __init__(self, inputs_dim=25):
        ...
        self.swish = Swish(inplace=False)
    ...
    def forward(self, inputs, targets=None):
        ...
        outputs = self.swish(outputs)
        outputs = self.swish(self.fc1(outputs)) # b,60,14
        outputs = self.swish(self.fc2(outputs))
...
if __name__ == "__main__":
    part0_input = np.load(os.path.join(adam_data_path, "adam_full/sample4_inputs_v4.npy"))
    part0_output = np.load(os.path.join(adam_data_path, "adam_full/sample4_outputs_v4.npy"))
