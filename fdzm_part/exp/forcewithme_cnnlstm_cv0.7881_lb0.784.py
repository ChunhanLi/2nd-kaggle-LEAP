...
config = {
    ...
    "evaluate_n_times_per_epoch_last": 10,
    "last_epoch_num": 1,
    "early_stop_flag": False,
    "early_stop_step": 1, # 1的话其实已经是两个epoch不增长了
    "batch_size": 2048,
    "val_batch_size": 2048*4,
    "eval_val": True,
    "num_workers": 40,
    "apex": False,
    "debug": False,
...
#     def __init__(self, inputs_dim=25, num_lstm=3, hidden_state=256):
#         super().__init__()
#         self.inputs_dim = inputs_dim
#         self.output_single_num = 8
        
#         self.convk1 = nn.Conv1d(inputs_dim, inputs_dim, kernel_size=1, padding='same')  # 25
#         self.convk3 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=3, padding='same')  # 75
#         self.convk5 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=5, padding='same')  # 125
#         self.convk7 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=7, padding='same')  # 175
#         self.convk11 = nn.Conv1d(inputs_dim, inputs_dim*3, kernel_size=11, padding='same')  # 250
#         self.convk19 = nn.Conv1d(inputs_dim, inputs_dim*3, kernel_size=19, padding='same')  # 325 
#         self.convk31 = nn.Conv1d(inputs_dim, inputs_dim*3, kernel_size=31, padding='same')  # 400 
#         self.convk60 = nn.Conv1d(inputs_dim, 87, kernel_size=60, padding='same')  # 487
...
#                                                embed_dim=hidden_state, first_layer=True if i==0 else False)
...
#         conv_enck1 = self.enc_act(self.convk1(inputs).permute(0,2,1))
#         conv_enck3 = self.enc_act(self.convk3(inputs).permute(0,2,1))
#         conv_enck5 = self.enc_act(self.convk5(inputs).permute(0,2,1))
#         conv_enck7 = self.enc_act(self.convk7(inputs).permute(0,2,1))
#         conv_enck11 = self.enc_act(self.convk11(inputs).permute(0,2,1))
#         conv_enck19 = self.enc_act(self.convk19(inputs).permute(0,2,1))
#         conv_enck31 = self.enc_act(self.convk31(inputs).permute(0,2,1))
#         conv_enck60 = self.enc_act(self.convk60(inputs).permute(0,2,1))

#         inputs = inputs.permute(0,2,1)
#         inputs = torch.concat([inputs, conv_enck1, conv_enck3, conv_enck5, conv_enck7, conv_enck11, conv_enck19, conv_enck31, conv_enck60], dim=-1)
#         inputs = self.enc_norm(inputs)
#         outputs = inputs
...
class LeapModel(nn.Module):
    def __init__(self, inputs_dim=25, num_lstm=3, hidden_state=256):
        ...
        self.convk1 = nn.Conv1d(inputs_dim, inputs_dim, kernel_size=1, padding='same')  # 50
        self.convk3 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=3, padding='same')  # 100
        self.convk5 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=5, padding='same')  # 150
        self.convk7 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=7, padding='same')  # 200
        self.convk11 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=11, padding='same')  # 250
        self.convk19 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=19, padding='same')  # 300
        self.lstm_enc = nn.LSTM(input_size=inputs_dim, hidden_size=106, num_layers=1, bidirectional=True, batch_first=True)
        # self.convk31 = nn.Conv1d(inputs_dim, inputs_dim*4, kernel_size=31, padding='same')  # 450
        # self.convk45 = nn.Conv1d(inputs_dim, inputs_dim*2, kernel_size=45, padding='same')  # 500
        # self.convk60 = nn.Conv1d(inputs_dim, 12, kernel_size=60, padding='same')  # 512
        self.enc_norm = nn.LayerNorm(512)
        self.enc_act = nn.GELU()
        self.lstm_layers = nn.ModuleDict()
        # 定义LSTM层
    ...
    def forward(self, inputs):
        lstm_out, _ = self.lstm_enc(inputs)
        inputs = inputs.permute(0,2,1)
        ...
        conv_enck19 = self.enc_act(self.convk19(inputs).permute(0,2,1))
        # conv_enck31 = self.enc_act(self.convk31(inputs).permute(0,2,1))
        # conv_enck45 = self.enc_act(self.convk45(inputs).permute(0,2,1))
        # conv_enck60 = self.enc_act(self.convk60(inputs).permute(0,2,1))
        inputs = inputs.permute(0,2,1)
        inputs = torch.concat([inputs, conv_enck1, conv_enck3, conv_enck5, conv_enck7, conv_enck11, conv_enck19, lstm_out], dim=-1)
        inputs = self.enc_norm(inputs)
        outputs = inputs
