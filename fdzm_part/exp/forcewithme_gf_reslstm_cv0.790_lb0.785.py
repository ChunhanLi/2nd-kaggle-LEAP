...
config = {
    ...
    "cosine_n_cycles": 0.5,
    "learning_rate": 8e-5,
    "resume": "../outputs/forcewithme_reslstm_cv0.789_lb0.783/forcewithme_reslstm_cv0.789_lb0.783.pt",
}
...
class LeapModel(nn.Module):
    def __init__(self, inputs_dim=25, num_lstm=8, hidden_state=256):
        ...
        for i in range(num_lstm):  # 假设有5个LSTM层
            ...
            lstm_layers[lstm_key] = nn.LSTM(input_size=self.inputs_dim if i == 0 else hidden_state*2, 
                                hidden_size=hidden_state, num_layers=1, bidirectional=True, batch_first=True)
            if i % 2 == 0:  
                residual_layers.append(
                                        nn.Sequential(nn.LayerNorm([60, 512]),
                                                        nn.GELU()
                                                    )                       
                )
            else:
    ...
    def forward(self, inputs):
        ...
        for i, lstm in enumerate(self.lstm_stack.values()):
            ...
            if i % 2 == 1:  
                outputs = self.res_act(outputs + last_outputs)  # 残差连接
